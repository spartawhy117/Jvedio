...
```
{repo-root}/log/
├── worker-2026-03-19.log     ← Worker (.NET) 日志
├── shell-2026-03-19.log      ← Tauri Shell (Rust) 日志
└── .gitkeep                  ← 保持目录在 Git 中
```
...
| 文件路径 | `{repo}/log/worker-{yyyy-MM-dd}.log` |
...
| 路径解析 | `ResolveLogDirectory()` — 优先读 `JVEDIO_LOG_DIR` 环境变量，再尝试从 exe 路径向上查找 repo 根目录 |
...
| 文件路径 | `{repo}/log/shell-{yyyy-MM-dd}.log` |
...
| 路径解析 | 编译时 `CARGO_MANIFEST_DIR` → 向上 2 级到 repo 根目录；`JVEDIO_LOG_DIR` 环境变量覆盖 |
...
| 备注 | WPF 主程序在 Release 模式下优先启动 Tauri Shell，自身日志主要用于 fallback 场景 |

## 3. 环境变量

| 变量 | 作用 |
|------|------|
| `JVEDIO_LOG_DIR` | 覆盖日志目录路径（Worker + Shell 均读取） |
...
```powershell
# 实时查看 Worker 日志
Get-Content -Path log/worker-*.log -Wait -Tail 50

# 实时查看 Shell 日志
Get-Content -Path log/shell-*.log -Wait -Tail 50

# 同时查看两个
Get-Content -Path log/*.log -Wait -Tail 50
```

### 5.2 Playwright 自动化测试

Playwright 测试期间，通过查看 `log/` 目录即可诊断所有后端和 Shell 问题：
- API 错误 → `worker-{date}.log` 中搜索 `[ERR]` 或 `[WRN]`
- Worker 启动失败 → `shell-{date}.log` 中搜索 `ERROR`
- Worker stdout 捕获问题 → `shell-{date}.log` 中搜索 `[Worker:stdout]`

### 5.3 生产环境

Release 模式下日志文件位于 Worker/Shell 可执行文件旁边的 `log/` 目录（或通过 `JVEDIO_LOG_DIR` 指定）。
