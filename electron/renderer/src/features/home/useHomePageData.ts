...
export class HomePageController {
  ...
  public constructor(rootElement: HTMLElement) {
    ...
    this.rootElement.addEventListener("keydown", (event) => this.handleKeyDown(event));
  ...
  private handleKeyDown(event: KeyboardEvent): void {
    ...
    const target = event.target;
    if (!(target instanceof HTMLInputElement) || target.tagName === "TEXTAREA") return;

    const libraryId = target.dataset.libraryId ?? "";
    if (target.dataset.queryField === "keyword" && libraryId) {
      event.preventDefault();
      window.location.hash = toHash({ kind: "library", libraryId, query: this.getLibraryVideoDraft(libraryId) });
      return;
    }

    if (target.dataset.favoritesQueryField === "keyword") {
      event.preventDefault();
      window.location.hash = toHash({ kind: "favorites", query: this.state.favoritesQueryDraft });
      return;
    }

    if (target.dataset.categoryQueryField === "keyword") {
      event.preventDefault();
      window.location.hash = toHash({
        kind: "category",
        name: this.state.route.kind === "category" ? this.state.route.name : "",
        query: this.state.categoryQueryDraft,
      });
      return;
    }

    if (target.dataset.seriesQueryField === "keyword") {
      event.preventDefault();
      window.location.hash = toHash({
        kind: "series",
        name: this.state.route.kind === "series" ? this.state.route.name : "",
        query: this.state.seriesQueryDraft,
      });
    }
  }
...
function renderHome(libraries: readonly LibraryListItemDto[], summary: TaskSummaryDto, worker: WorkerStatusDto, tasks: readonly WorkerTaskDto[], pendingRetryTaskId: string): string {
  return `<section class="metric-grid">${metric("媒体库", String(libraries.length), "Home 与左导航共享库清单")}${metric("运行中任务", String(summary.runningCount), "包含扫描与抓取任务")}${metric("今日完成", String(summary.completedTodayCount), "由 SSE 与 /api/tasks 刷新")}${metric("Worker", worker.healthy ? "Healthy" : "Unavailable", worker.baseUrl || "等待 Worker 地址")}</section><section class="split-layout"><div class="surface-card"><div class="section-header"><div><span class="eyebrow">Libraries</span><h2>媒体库列表</h2></div><button class="ghost-button" data-action="open-create-dialog">添加媒体库</button></div><div class="collection-summary-strip"><div class="collection-summary-copy"><span class="note-label">工作区入口</span><div class="collection-summary-title">从 Home 可以直接切到 Favorites、Categories、Series、Actors 和 Settings。</div><div class="footer-hint">这些页面现在共用同一套影片卡片、筛选摘要和返回链路。</div></div><div class="collection-summary-pills"><a class="summary-pill summary-link-pill" href="${toHash({ kind: "favorites", query: createDefaultLibraryVideoRouteQuery() })}">Favorites</a><a class="summary-pill summary-link-pill" href="${toHash({ kind: "category", name: "", query: createDefaultLibraryVideoRouteQuery() })}">Categories</a><a class="summary-pill summary-link-pill" href="${toHash({ kind: "series", name: "", query: createDefaultLibraryVideoRouteQuery() })}">Series</a><a class="summary-pill summary-link-pill" href="${toHash({ kind: "actors", query: createDefaultActorsRouteQuery() })}">Actors</a><a class="summary-pill summary-link-pill" href="${toHash({ kind: "settings", group: "basic" })}">Settings</a></div></div>${libraries.length > 0 ? `<div class="library-grid">${libraries.map((library) => renderLibraryCard(library, tasks)).join("")}</div>` : `<div class="empty-card"><h3>还没有媒体库</h3><p>先创建一个库，再在库页完成扫描、展示和播放验证。</p></div>`}</div><div class="surface-card side-stack"><div class="section-header"><div><span class="eyebrow">Task Summary</span><h2>任务摘要</h2></div></div><div class="collection-summary-strip"><div class="collection-summary-copy"><span class="note-label">摘要视图</span><div class="collection-summary-title">任务状态、失败重试与活动条在 Home / Library 使用同一套语义。</div><div class="footer-hint">失败任务可直接查看详情并重试，活动条会跨页面保留当前焦点任务。</div></div><div class="collection-summary-pills"><span class="summary-pill">运行中 ${summary.runningCount}</span><span class="summary-pill">排队中 ${summary.queuedCount}</span><span class="summary-pill">失败 ${summary.failedCount}</span><span class="summary-pill">今日完成 ${summary.completedTodayCount}</span></div></div><div class="task-list">${row("运行中", summary.runningCount)}${row("排队中", summary.queuedCount)}${row("失败", summary.failedCount)}${row("今日完成", summary.completedTodayCount)}</div><div class="task-feed home-task-feed">${tasks.length > 0 ? tasks.slice(0, 5).map((task) => renderTaskCard(task, pendingRetryTaskId)).join("") : `<div class="empty-task-feed">当前还没有任务记录。</div>`}</div><div class="worker-note" data-last-updated-utc="${escapeHtml(summary.lastUpdatedUtc)}"><div class="note-label">最近刷新</div><div class="note-value">${formatDate(summary.lastUpdatedUtc)}</div></div></div></section>`;
}
...
function renderVideoResultCard(video: VideoListItemDto, backToHash: string | null): string {
  ...
  return `<article class="video-result-card ${video.hasMissingAssets ? "has-gaps" : ""}" data-video-id="${escapeHtml(video.videoId)}" data-video-has-missing-assets="${video.hasMissingAssets ? "true" : "false"}"><div class="video-result-head"><a href="${detailHash}" class="video-result-title">${escapeHtml(video.displayTitle)}</a><span class="code-pill">${escapeHtml(video.vid || "NO-VID")}</span></div><div class="video-result-meta"><span>扫描：${escapeHtml(video.lastScanAt ? formatDate(video.lastScanAt) : "未记录")}</span><span>播放：${escapeHtml(video.lastPlayedAt ? formatDate(video.lastPlayedAt) : "未播放")}</span><span>次数：${video.viewCount}</span></div><div class="badge-row">${asset("NFO", video.hasNfo)}${asset("Poster", video.hasPoster)}${asset("Thumb", video.hasThumb)}${asset("Fanart", video.hasFanart)}</div><div class="video-card-note">${escapeHtml(video.hasMissingAssets ? "当前影片仍缺少部分 sidecar 或图片资源。" : "sidecar 与图片资源状态完整。")}</div><div class="inline-note">${escapeHtml(video.path)}</div><div class="library-card-actions"><a href="${detailHash}" class="ghost-link">查看详情</a></div></article>`;
}
...
function renderCollectionSummaryStrip(args: {
  draft: LibraryVideoRouteQuery;
  note: string;
  ...
  const { draft, note, selectionLabel, selectionValue, totalCount } = args;
  const keyword = draft.keyword.trim().length > 0 ? draft.keyword.trim() : "全部影片";
  const scope = draft.missingSidecarOnly ? "仅缺 sidecar" : "全部资源";
  return `<div class="collection-summary-strip"><div class="collection-summary-copy"><span class="note-label">结果摘要</span><div class="collection-summary-title">${escapeHtml(selectionLabel)} · ${escapeHtml(selectionValue)}</div><div class="footer-hint">${escapeHtml(note)}</div></div><div class="collection-summary-pills"><span class="summary-pill">${totalCount} 项</span><span class="summary-pill">关键字 ${escapeHtml(keyword)}</span><span class="summary-pill">${escapeHtml(scope)}</span><span class="summary-pill">排序 ${escapeHtml(videoSortLabel(draft.sortBy))} / ${draft.sortOrder === "asc" ? "升序" : "降序"}</span></div></div>`;
}
...
function videoSortLabel(sortBy: string): string { return sortBy === "title" ? "标题" : sortBy === "vid" ? "VID" : sortBy === "releaseDate" ? "发行日期" : sortBy === "lastPlayedAt" ? "最近播放" : sortBy === "viewCount" ? "播放次数" : "最近扫描"; }
