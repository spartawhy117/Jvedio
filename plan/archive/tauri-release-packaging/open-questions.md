...
**背景**：当前 `tauri.conf.json` 版本为 `0.1.0`，`package.json` 也是 `0.1.0`。

**最终结论**：版本号从 `5.0.0` 重设为 `0.1.0`。Tauri 架构是全新重写，与旧 WPF `Jvedio 4.6.x` 无延续关系，采用语义化版本 `0.x.y` 表达"已可用但仍在快速迭代"。自动更新不在本期范围。

**版本管理约定**：
- 版本号遵循 SemVer（MAJOR.MINOR.PATCH）
- 当前使用 `0.x.y` 阶段
- 每次发布修改 4 个核心文件 → 打 git tag `v{MAJOR}.{MINOR}.{PATCH}`
- CHANGELOG 切割 `[未发布]` → 版本区块

**状态**：✅ 已决定 — 重设为 `0.1.0`，已打 tag `v0.1.0`
