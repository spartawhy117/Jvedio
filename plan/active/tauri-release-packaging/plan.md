...
**目标**：将散落在 `tauri/` 子树多个深层目录的构建产物统一收归到仓库根 `build/` 目录，与 `log/`、`test-data/` 保持一致的组织风格。

**问题分析**：

| # | 问题 | 说明 |
|---|------|------|
| 1 | 路径层级太深 | 安装包在 `tauri/src-tauri/target/release/bundle/nsis/`，6 层 |
| 2 | 中间产物与源码混杂 | `worker-dist/`、`worker-publish/`、`dist/` 都在 `tauri/` 源码目录下 |
| 3 | 残留旧产物 | bundle 里有旧名称 `Jvedio_5.0.0` 的残留 msi 和 nsis |
| 4 | 没有统一出口 | 开发者要找安装包需记住完整深层路径 |
| 5 | 与 log/test-data 风格不一致 | 它们在仓库根、按职能分层；构建产物没有 |

**新目录结构**：

```
{repo-root}/build/                    ← 统一构建输出根目录（全部 gitignore）
├── release/                          ← 最终发布产物（安装包 copy 到此）
│   └── JvedioNext_5.0.0_x64-setup.exe
...
Before:
  npm run build:release
    → worker 暂存到 tauri/worker-dist/          （tauri/ 子树内）
    → 前端编译到 tauri/dist/                      （tauri/ 子树内）
    → 安装包在 tauri/src-tauri/target/release/bundle/nsis/  （6 层深）

After:
  npm run build:release
    → worker 暂存到 build/worker-stage/           （仓库根 1 层）
    → 前端编译到 build/frontend-stage/             （仓库根 1 层）
    → 安装包自动 copy 到 build/release/            （仓库根 1 层）
```
...
在修改配置和脚本之前，先清理已有的散落构建内容和目录，避免迁移后新旧产物并存混淆。

**当前散落产物盘点**：

| 目录 | 状态 | 文件数 | 大小 | 说明 |
|------|------|--------|------|------|
| `tauri/worker-dist/` | 存在 | 26 | 2.83 MB | Worker dotnet publish 产物（.exe + .dll + .pdb + .json + .config） |
| `tauri/dist/` | 存在 | 4 | 0.35 MB | 前端 vite build 产物（index.html + assets） |
| `tauri/worker-publish/` | 不存在 | — | — | 已被清理，无需处理 |
| `tauri/src-tauri/target/release/bundle/nsis/` | 存在 | 2 | ~7 MB | 当前 + 旧名安装包残留 |
| `tauri/src-tauri/target/release/bundle/msi/` | 存在 | 1 | ~0.7 MB | 旧 MSI 残留（已弃用格式） |

**清理操作清单**：

| # | 操作 | 命令/方式 | 说明 |
|---|------|-----------|------|
| 0.1 | 删除 `tauri/worker-dist/` 全部内容 | `Remove-Item tauri/worker-dist/* -Recurse -Force` | 26 个文件，2.83 MB；新构建将产出到 `build/worker-stage/` |
| 0.2 | 删除 `tauri/worker-dist/` 目录本身 | `Remove-Item tauri/worker-dist -Recurse -Force` | 目录不再需要（.gitignore 规则也将移除） |
| 0.3 | 删除 `tauri/dist/` 全部内容及目录 | `Remove-Item tauri/dist -Recurse -Force` | 4 个文件，0.35 MB；新构建将产出到 `build/frontend-stage/` |
| 0.4 | 删除旧名安装包 `Jvedio_5.0.0_x64-setup.exe` | `Remove-Item .../bundle/nsis/Jvedio_5.0.0_x64-setup.exe` | 旧命名残留，当前正式名是 `JvedioNext_*` |
| 0.5 | 删除旧 MSI 残留 `Jvedio_5.0.0_x64_en-US.msi` | `Remove-Item .../bundle/msi/Jvedio_5.0.0_x64_en-US.msi` | 已弃用的 MSI 格式残留 |
| 0.6 | 删除空的 `bundle/msi/` 目录 | `Remove-Item .../bundle/msi -Recurse -Force` | MSI 格式已弃用，清理空目录 |
| 0.7 | 创建 `build/` 目录骨架 | `mkdir build/release, build/worker-stage, build/frontend-stage` | 为后续步骤准备目标目录 |

**执行时机**：在 Step 7.1~7.8（核心配置改动）**之前**执行，确保旧产物不干扰新构建链路。

**注意事项**：
- `tauri/src-tauri/target/release/bundle/nsis/JvedioNext_5.0.0_x64-setup.exe`（当前正式安装包）暂保留，作为迁移后的对比基准
- 清理完成后立即执行 Step 7.1~7.8，然后重新 `build:release` 验证新链路
- 清理脚本可集成到 `tauri/scripts/clean-legacy-build.ps1`，也可手动一次性执行
...
| # | 文件 | 类型 | 动作 | 说明 |
|---|------|------|------|------|
| 7.1 | `.gitignore`（根） | 配置 | 添加 | 添加 `build/` 忽略规则 |
| 7.2 | `tauri/.gitignore` | 配置 | 修改 | 删除 `worker-dist/*`、`!worker-dist/.gitkeep`、`worker-publish/` 规则（不再需要） |
| 7.3 | `tauri/scripts/prepare-worker.ps1` | 脚本 | 修改 | `$publishOut` → `build/worker-stage`（直接 publish 到最终位置，不再中转）；删除 `worker-dist` 中转步骤和 `worker-publish` 清理步骤 |
| 7.4 | `tauri/src-tauri/tauri.conf.json` | 配置 | 修改 | `bundle.resources` 从 `"../worker-dist/**/*": "worker/"` 改为 `"../../build/worker-stage/**/*": "worker/"` |
| 7.5 | `tauri/src-tauri/tauri.conf.json` | 配置 | 修改 | `frontendDist` 从 `"../dist"` 改为 `"../../build/frontend-stage"` |
| 7.6 | `tauri/vite.config.ts` | 配置 | 修改 | 添加 `build.outDir: "../build/frontend-stage"` |
| 7.7 | 新增 `tauri/scripts/copy-release.ps1` | 脚本 | 新建 | 打包完成后将安装包从 `tauri/src-tauri/target/release/bundle/nsis/` copy 到 `build/release/` |
| 7.8 | `tauri/package.json` | 配置 | 修改 | `build:release` 脚本末尾追加 `copy-release` 步骤；`prepare-worker` 脚本路径不变 |

**第二轮：关联代码检查**

| # | 文件 | 类型 | 动作 | 说明 |
|---|------|------|------|------|
| 7.9 | `tauri/src-tauri/src/worker.rs` | Rust | **检查** | `resolve_worker_path()` Dev 模式使用 `CARGO_MANIFEST_DIR` 相对路径 → 不受影响；Release 模式用 `exe_dir/worker/` → 不受影响（安装包内结构不变） |
| 7.10 | `tauri/src-tauri/src/shell_log.rs` | Rust | **检查** | 日志路径从 `CARGO_MANIFEST_DIR` 推断 → 不受影响 |
| 7.11 | `tauri/src-tauri/src/lib.rs` | Rust | **检查** | Tauri `beforeBuildCommand` 执行 `npm run build` → vite 会读 `vite.config.ts` 的 `outDir` → 如果 7.6 改了，Tauri 内部的 `frontendDist` 也要对应改 → 已在 7.5 覆盖 |
| 7.12 | `dotnet/Jvedio.Worker/Services/WorkerPathResolver.cs` | C# | **检查** | 不涉及构建路径 → 不受影响 |
| 7.13 | `tauri/.gitignore` | 配置 | **检查** | `dist` 规则也要移除（前端产物已不在 `tauri/dist/`）→ 已在 7.2 覆盖 |

**第三轮：文档更新**

| # | 文件 | 类型 | 动作 | 说明 |
|---|------|------|------|------|
| 7.14 | `AGENTS.md` | 文档 | 修改 | 产出安装包路径改为 `build/release/JvedioNext_*-setup.exe` |
| 7.15 | `doc/data-directory-convention.md` | 文档 | 修改 | 新增 §8「构建输出目录」章节，说明 `build/` 结构和用途 |
| 7.16 | `doc/modules/13-tauri-shell-worker.md` | 文档 | 修改 | 更新 `bundle.resources` 代码片段和「构建脚本」行的路径说明 |
| 7.17 | `doc/CHANGELOG.md` | 文档 | 追加 | 记录统一构建输出目录变更 |
| 7.18 | `doc/logging-convention.md` | 文档 | **检查** | 不涉及构建路径 → 无需改动 |
| 7.19 | `plan/active/tauri-release-packaging/handoff.md` | 文档 | 修改 | 更新 D-6 冻结决策（`prepare-worker.ps1 → build/worker-stage/ → bundle.resources`） |
| 7.20 | `plan/active/tauri-release-packaging/open-questions.md` | 文档 | 修改 | 更新 OQ-5（`.gitignore` 覆盖）的状态和说明 |
| 7.21 | `plan/active/tauri-release-packaging/validation.md` | 文档 | 修改 | 更新 1.2/1.3/4.3/6.3 中的 `worker-dist` / `bundle/nsis` 路径为新路径 |

**验证步骤**：

0. **Step 0 清理验证**：确认 `tauri/worker-dist/`、`tauri/dist/`、`bundle/msi/` 不存在；确认 `build/` 三个子目录已创建
1. `npm run prepare-worker` — 确认 `build/worker-stage/Jvedio.Worker.exe` 产物完整
2. `npm run build`（在 `tauri/` 下） — 确认 `build/frontend-stage/index.html` 存在
3. `npm run build:release` — 确认 NSIS 安装包产出且被 copy 到 `build/release/`
4. `git check-ignore build/release/test.txt build/worker-stage/test.txt build/frontend-stage/test.txt` — 确认全部被忽略
5. 确认 `tauri/worker-dist/`、`tauri/dist/` 没有重新生成

**通过标准**：

- [ ] **Step 0 清理**：`tauri/worker-dist/` 目录不存在
- [ ] **Step 0 清理**：`tauri/dist/` 目录不存在
- [ ] **Step 0 清理**：`bundle/msi/` 目录不存在，旧名 `Jvedio_5.0.0_x64-setup.exe` 不存在
- [ ] **Step 0 清理**：`build/release/`、`build/worker-stage/`、`build/frontend-stage/` 目录已创建
- [ ] `build/release/JvedioNext_5.0.0_x64-setup.exe` 存在且与 bundle 目录下一致
- [ ] `build/worker-stage/Jvedio.Worker.exe` 存在
- [ ] `build/frontend-stage/index.html` 存在
- [ ] `.gitignore` 覆盖 `build/` 全目录
- [ ] 所有文档已同步（7.14~7.21）
- [ ] 安装包安装后启动链路正常（回归验证）
