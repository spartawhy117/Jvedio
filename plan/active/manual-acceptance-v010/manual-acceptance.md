...
- 最新调整已进一步收口为“纯箭头返回按钮”，不再在界面上展示任何返回文案，避免详情页头部再次被长文本占用。
...
### F-025 返回按钮不应再展示任何附加文本

已核对证据：
- 当前真包截图已经证明，即便把返回文案语义改成“目标页标题”，详情页头部依旧会被一整段返回文本干扰，实际体验不符合用户预期。
- [BackNavigation.tsx](D:/study/Proj/Jvedio/tauri/src/components/shared/BackNavigation.tsx) 仍然渲染了 `.back-navigation-label` 文本节点，因此任何历史 label / fallbackLabel 都会变成可见内容。
- [BackNavigation.css](D:/study/Proj/Jvedio/tauri/src/components/shared/BackNavigation.css) 虽然做了省略，但本质上仍是“带文字的胶囊返回组件”，这和当前详情页需要的“纯图标返回”不是一类控件。

初步根因判断：
- 这已经不是“文案语义是否正确”的问题，而是组件形态本身选错了。详情页头部需要的是单纯的返回 affordance，不需要任何并排文字。
- 只要继续保留可见 label，不论它是当前标题、目标页标题还是截断文本，都会占据头部横向空间并放大视觉噪音。
...
后续修复方向：
- 将返回组件收口为纯箭头按钮，删除界面上的文字展示框。
- tooltip / aria-label 可以保留基础辅助信息，但不再参与可见布局。
...
| F-025 | 人工验收 | 详情页顶部返回按钮后面的文字仍然过长，当前应直接删除文字展示框，只保留返回箭头 | 当前 `BackNavigation` 仍保留可见 label 区域，只要存在 fallback 或历史 label，就会继续占据页面头部宽度 | 已改为纯箭头返回按钮，界面上不再渲染文字展示框；下一步只需确认真包观感 | 已修复待复验 |
