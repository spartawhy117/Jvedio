...
- 第二批阶段 `D`、第三批“影片展示和播放”、第四批“设置页面”第一轮最小闭环和设置消费扩展均已完成实现与验证；演员页当前也已完成第二轮增强收口：Actors 路由壳、列表结果集、筛选排序、关联影片抽屉、演员详情头部消费、头像真实路径解析 / 占位策略、结果分页和扩展排序均已接通。后续执行顺序已冻结为：`1. 右侧内容区独立演员详情页`、`2. Settings 第二轮页签对齐`、`3. Favorites`、`4. 智能分类（类别 / 系列）`。
...
1. 演员详情页：
   - 在右侧内容区补独立 `Actor Detail` 路由页，不开新窗口
   - 打通 `Actors -> Actor Detail -> Video Detail` 和稳定返回链路
   - 将当前抽屉能力逐步收口为详情页主形态
2. Settings 第二轮：
   - 页签数量与当前既有设置页保持一致
   - 每个页签按当前已有开关 / 输入项呈现
